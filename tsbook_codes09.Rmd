...
#An introduction and analysis examples of a well-known component model in the linear Gaussian state space model

##Combination of individual model
...
# <<Generate artificial data obeyed local level model>>
...
# Find 25% and 75% values for 50% intervals of the filtering distribution
...
# Legend
legend(legend = c("Observations", "Mean (filtering distribution)", "50% intervals (filtering distribution)"),
...
# Find 25% and 75% values for 50% intervals of the predictive distribution
...
# Legend
legend(legend = c("Observations", "Mean (predictive distribution)", "50% intervals (predictive distribution)"),
       lty = c("solid", "solid", "dashed"),
       col = c("lightgray", "black", "black"),
       x = "topright", text.width = 70, cex = 0.6)
...
# Find 25% and 75% values for 50% intervals of the smoothing distribution
...
# Legend
legend(legend = c("Observations", "Mean (smoothing distribution)", "50% intervals (smoothing distribution)"),
       lty = c("solid", "solid", "dashed"),
       col = c("lightgray", "black", "black"),
       x = "topright", text.width = 70, cex = 0.6)
...
###Approach from time domain

###Approach from frequency domain
...
# Kalman filtering
dlmFiltered_obj  <- dlmFilter(y = y, mod = mod)
dlmFiltered_obja <- dlmFiltered_obj              # Save as a different name for the later comparison of prediction values
...
# Kalman filtering
dlmFiltered_obj  <- dlmFilter(y = y, mod = mod)
dlmFiltered_objb <- dlmFiltered_obj              # Save as a different name for the later comparison of prediction values
...
# Kalman filtering
dlmFiltered_obj  <- dlmFilter(y = y, mod = mod)
dlmFiltered_objc <- dlmFiltered_obj              # Save as a different name for the later comparison of prediction values
...
# Find the standard deviation, 2.5%, and 97.5% values of the prediction value
...
# Unite whole observations along with mean, 2.5%, and 97.5% of prediction values into ts class object
...
# <<Comparison of forecasts from 2015 among 3 models>>

# Find mean, 2.5%, and 97.5% values of the prediction values for each of model a, b, and c
...
# Unite the prediction results for each model into ts class
names(f_all) <- c("a", "b", "c")
y_pred <- do.call("ts.union", f_all)

# Extract 2015 data from whole observations
...
# <<Comparison of MAPEs from 2015 among 3 models>>
...
# Legend
legend(legend = c("Mean (smoothing distribution)", "50% intervals (smoothing distribution)"),
       lty = c("solid", "dashed"),
       col = c("black", "black"),
       x = "topleft", cex = 0.6)
...
# 50% intervals of level estimator (finding 25% and 75% values)
...
# Legend
legend(legend = c("Observations", "Mean", "50% intervals"),
       lty = c("solid", "solid", "dashed"),
       col = c("lightgray", "black", "black"),
       x = "topright", cex = 0.6)
```
...
x_lab[!(x_lab %in% c("Saturday", "Sunday"))] <- ""
mtext(text =  x_lab, side = 1, line = 1, at = food_day$Date)
mtext(text =  "1st day", side = 1, line = 1, at = as.Date("2009-06-01"))
mtext(text = "30th day", side = 1, line = 1, at = as.Date("2009-06-30"))
```
...
jholidays <- function(days){
  ...
  # Regard Saturdays, Sundays, and public holidays (including its compensations) as holidays
...
# Aggregate the number of weekdays or holidays per every month
monthly <- table(substr(days, start = 1, stop = 7), jholidays(days))  

# Explanatory variable (total number of weekdays in a month minus that of holidays)
...
# Bind explanatory variables (weekday effect and leap year effect)
